...
const FishItem = ({ item, data = Map(), gameId }) => {
  ...
  const bonusTotalPay = () => {
    const array = item.getIn([1, "BonusInfos"]).toJS();
    let totalPay = 0;
    for (let i = 0; i < array.length; i++) {
      totalPay += array[i]["Pay"] * 100;
    }
    totalPay += item.getIn([1, "Pay"]) * 100;
    return totalPay / 100;
  };
  return (
    <>
      <Box>
        ...
        {item.getIn([1, "PayEx"]) !== "0.00" && (
          <p className="extra-payout-column">
            {t("extraPayout")}：{item.getIn([1, "PayEx"]) * 1}
          </p>
        )}
      </Box>
      {open && (
