...
const WeaponTable = ({
  ...
  const [weaponList, setWeaponList] = useState(List());
  ...
  const popupHandler = (id) => {
    ...
    setPopupList(
      list
        .filter((i) => i.get("Serial") == id)
        .sort((a, b) => {
          return a.get("BetMultiple") - b.get("BetMultiple");
        })
    );
  };
  ...
  const fishImg = (type) => {
    if (type == "Glove") {
      return `${imgsdomain}/order-detail/common/GO02/22.png`;
    } else {
      return `${imgsdomain}/order-detail/common/GO02/23.png`;
    }
  };
  const totalPay = () => {
    let total = 0;
    ...
    if (antesValue === "all") {
      ...
      list.toJS().forEach((item) => {
        ...
        if (index !== -1) {
          ...
          if (item["WeaponKill"]) {
            const weaponKill = item["WeaponKill"];
            newArr[index]["WeaponKill"] = weaponKill;
          }
                      ...
                      <div className="tr" key={idx}>
                        ...
                        {item.get("Win") > 0 ? (
                          <div
                            className="td more"
                            onClick={() => {
                              thirdPopupHandler(item.get("WeaponKill"));
                            }}
                          >
                            <div className="dot"></div>
                            <div className="dot"></div>
