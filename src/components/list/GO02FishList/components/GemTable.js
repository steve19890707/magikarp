...
const GemTable = ({ list = List(), antesValue, denom, size, setIsBgFixed }) => {
  ...
  const popupHandler = (id) => {
    const newList = list.filter((i) => i.get("Serial") == id);
    const newArr = [];
  ...
  // create gem display list
  useEffect(() => {
    if (antesValue === "all") {
      const newArr = [];
      list.toJS().forEach((item) => {
        const index = newArr.findIndex((i) => i["Serial"] === item["Serial"]);
        if (index !== -1) {
          const bullets = newArr[index]["Bullets"] + item["Bullets"];
          const bulletsAwake1 =
            newArr[index]["BulletsAwake1"] + item["BulletsAwake1"];
  ...
  return (
    <>
      <Main>
        <div className="left">
          <img src={`${imgsdomain}/order-detail/common/GO02/21_1.png`}></img>
        </div>
...
${imgsdomain}/order-detail/common/GO02/character4_pc.png`}
                  ...
                  <div className="th">{t("externalMultiplier")}</div>
                  <div className="th">{t("internalMultiplier")}</div>
                  <div className="th">{t("payout")}</div>
                </div>
                <div className="tbody">
                  {popupList.map((item, idx) => (
                    <div className="tr" key={idx}>
                      ...
                      <div className="td">{item.get("BulletsAwake4")}</div>
                      <div className="td">
                        {item.get("OuterMultiplier") || "-"}
                      </div>
                      <div className="td">
                        {item.get("InnerMultiplier") || "-"}
                      </div>
                      <div className="td">
                        {(item.get("Win") * (denom * 100)) / 100}
                      </div>
...
${imgsdomain}/order-detail/common/GO02/character4_mobile.png`}
                  ...
                  <div className="tr">
                    ...
                    <div
                      className="more"
                      onClick={() => popupHandler(item.get("Serial"))}
                    >
                      {t("more")}
                    </div>
                  </div>
...
${imgsdomain}/order-detail/common/GO02/character4_mobile.png`}
                        ...
                        <div className="tr">
                          <h5>{t("externalMultiplier")}</h5>
                          <p>{item.get("OuterMultiplier") || "-"}</p>
                        </div>
                        <div className="tr">
                          <h5>{t("internalMultiplier")}</h5>
                          <p>{item.get("InnerMultiplier") || "-"}</p>
                        </div>
                        <div className="tr">
                          <h5>{t("payout")}</h5>
                          <p>{(item.get("Win") * (denom * 100)) / 100}</p>
