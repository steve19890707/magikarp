...
import ReframingAT05AntesSelectBar from "components/list/ReframingAT05AntesSelectBar";
...
import CannonTable from "./components/CannonTable";

const ReframingAT05List = ({ data, list = List() }) => {
  ...
  const [cannonList, setCannonList] = useState(List());
  ...
  useEffect(() => {
    if (antesValue === "all") {
      ...
      if (bets) {
        ...
        setCannonList(List(list.get("Weapons")));
      }
    } else {
      // 單一底注
      const bets = list.getIn(["Bets", antesValue, "Fishes"]);
      // cannon One Ante
      const weaponsList = List(list.get("Weapons"));
      let cannonOneAnte = weaponsList;
      weaponsList.forEach((element, key) => {
        element[1].get("Bets").forEach((_, ObjectKey) => {
          if (ObjectKey !== antesValue) {
            cannonOneAnte = removeIn(cannonOneAnte, [
              key,
              1,
              "Bets",
              ObjectKey,
      ...
      if (bets) {
        ...
        setCannonList(cannonOneAnte.filter((v) => v[1].get("Bets").size > 0));
      }
    }
  }, [list, antesValue]);
  return (
...
`;
export default ReframingAT05List;
